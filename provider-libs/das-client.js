...
var client, tableName, encodedQuery, hostname, port, providerConfig, dataRowLimit, onSuccessFunction, schema,
    timeGap = -1, orderType, orderingField;

function subscribe(_encodedQuery, _tableName, onSuccessData, onErrorData, _hostname, _port, _providerConfig, _schema){
...
function startPoll(){
    (function poll(){
        setTimeout(function(){
            var decodedQuery;
            if(encodedQuery.indexOf("key=") !== -1) {
                decodedQuery= atob(encodedQuery.replace("key=", ""));
            } else {
                decodedQuery = "";
            }
            var query = buildDasQuery(decodedQuery, tableName, 0, 1);
            client.search(
                query,
                function (data2) {
                    ...
                    if(-1 != timeGap) {
                        ...
                        if("" == decodedQuery) {
                            decodedQuery = decodedQuery + "TimestampUTC=[" +startingTimestamp + " TO " + endingTimestamp + "]";
                        } else {
                            decodedQuery = decodedQuery + "&TimestampUTC=[" +startingTimestamp + " TO " + endingTimestamp + "]";
                        }
                    }
                    query = buildDasQuery(decodedQuery, tableName, 0, 1);
                    client.searchCount(
                        query,
                        function (data) {
                            var query = buildDasQuery(decodedQuery, tableName, 0, data.message);
                            client.search(
                                query,
                                function (data2) {
                                    var data = [];
